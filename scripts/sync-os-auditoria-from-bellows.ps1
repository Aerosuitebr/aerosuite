# Copia os_auditoria: bellows -> aerosuite (acentos corretos na origem).
# Uso: .\scripts\sync-os-auditoria-from-bellows.ps1

$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path -Parent $PSScriptRoot
$sqlFile = Join-Path $repoRoot 'db\scripts\sync_os_auditoria_bellows_to_aerosuite.sql'
...
Write-Host "Sincronizando os_auditoria: bellows -> aerosuite em ${dbHost}:${port} ..."
...
Write-Host 'Concluido. Atualize a tela de Auditoria de OS no browser (Ctrl+F5).'
