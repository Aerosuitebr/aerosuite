#!/usr/bin/env node
/**
 * Envia Confronto Relatório 6 à equipe (PDF + e-mail HTML).
 * Uso:
 *   node scripts/send-homolog-confronto-relatorio6-email.mjs --dry-run
 *   node scripts/send-homolog-confronto-relatorio6-email.mjs
 *   node scripts/send-homolog-confronto-relatorio6-email.mjs --to wellemlyra@aerosuite.com.br
 */
import fs from 'fs';
...
const { SCORE } = await import(
  `file://${path.join(root, 'docs/homolog_ux/confronto-relatorio6-data.mjs').replace(/\\/g, '/')}`
);

if (SCORE.pending > 0 || Number(SCORE.pctConformidadeCodigo) < 100) {
  console.error(
    `Envio bloqueado: ${SCORE.pending} pendente(s) no código, conformidade ${SCORE.pctConformidadeCodigo}%`,
  );
  process.exit(1);
}
...
const pdfPath = path.join(root, `docs/homolog_ux/Confronto_Homolog_UX_Relatorio6_${stamp}_verificado.pdf`);
const htmlPath = path.join(root, `docs/homolog_ux/emails/email-confronto-relatorio6-equipe-${stamp}.html`);

for (const f of [pdfPath, htmlPath]) {
  if (!fs.existsSync(f)) {
    console.error('Arquivo ausente:', f);
    console.error('Execute: node scripts/build-homolog-confronto-relatorio6-pdf.mjs');
    process.exit(1);
  }
}
...
const deployNote = SCORE.deployRequired ? ' · validação prod pós-deploy' : '';
const subject = `Confronto Técnico — Relatório 6 Homologação UX · 28/28 achados corrigidos (PDF em anexo)${deployNote}`;
...
if (dryRun) {
  console.log('Dry-run — conformidade código', SCORE.pctConformidadeCodigo + '%');
  console.log('Deploy pendente em produção:', SCORE.deployRequired);
...
const info = await transporter.sendMail({
  ...
  attachments: [
    ...logoAttachments,
    {
      filename: 'Confronto_Homolog_UX_Relatorio6_AeroSuite.pdf',
      path: pdfPath,
