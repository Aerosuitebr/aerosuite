# Copia tipo_servico: bellows -> aerosuite (acentos corretos na origem).
# Uso: .\scripts\sync-tipos-servico-from-bellows.ps1

$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path -Parent $PSScriptRoot
$sqlFile = Join-Path $repoRoot 'db\scripts\sync_tipo_servico_bellows_to_aerosuite.sql'
...
Write-Host "Sincronizando tipo_servico: bellows -> aerosuite em ${dbHost}:${port} ..."
...
elseif (Get-Command docker -ErrorAction SilentlyContinue) {
    ...
    $content | docker run --rm -i mysql:8.0 mysql --default-character-set=utf8mb4 -h $dockerHost -P $port -u $user "-p$pass" aerosuite
...
Write-Host 'Concluido. Atualize a lista de tipos de servico no browser (Ctrl+F5).'
