#!/usr/bin/env node
/**
 * Gera PDF — Previsão de Precificação Aero Suite
 * Uso: node scripts/build-previsao-precificacao-pdf.mjs
 */
import fs from 'fs';
...
const htmlSrc = path.join(outDir, 'Previsao_Precificacao_Aero_Suite.html');
const htmlOut = path.join(outDir, `Previsao_Precificacao_Aero_Suite_${stamp}.html`);
const pdfOut = path.join(outDir, `Previsao_Precificacao_Aero_Suite_${stamp}.pdf`);
