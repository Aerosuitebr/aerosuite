# Copia template_produto_servico: bellows -> aerosuite (acentos corretos na origem).
# Uso: .\scripts\sync-templates-from-bellows.ps1

$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path -Parent $PSScriptRoot
$sqlFile = Join-Path $repoRoot 'db\scripts\sync_template_produto_servico_bellows_to_aerosuite.sql'
...
Write-Host "Sincronizando templates: bellows -> aerosuite em ${dbHost}:${port} ..."
...
Write-Host 'Concluido. Atualize a lista de templates no browser (Ctrl+F5).'
