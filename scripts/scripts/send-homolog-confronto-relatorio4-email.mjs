#!/usr/bin/env node
/**
 * Envia Confronto Relatório 4 — somente se 100% conformidade (0 pendentes).
 * Uso:
 *   node scripts/send-homolog-confronto-relatorio4-email.mjs --dry-run
 *   node scripts/send-homolog-confronto-relatorio4-email.mjs
 *   node scripts/send-homolog-confronto-relatorio4-email.mjs --to wellemlyra@aerosuite.com.br
 */
import fs from 'fs';
...
const { SCORE } = await import(`file://${path.join(root, 'docs/homolog_ux/confronto-relatorio4-data.mjs').replace(/\\/g, '/')}`);
...
const pdfPath = path.join(root, `docs/homolog_ux/Confronto_Homolog_UX_Relatorio4_${stamp}_verificado.pdf`);
const htmlPath = path.join(root, `docs/homolog_ux/emails/email-confronto-relatorio4-equipe-${stamp}.html`);

for (const f of [pdfPath, htmlPath]) {
  if (!fs.existsSync(f)) {
    console.error('Arquivo ausente:', f);
    console.error('Execute: node scripts/build-homolog-confronto-relatorio4-pdf.mjs');
    process.exit(1);
  }
}
...
const subject =
  'Confronto Técnico — Relatório 4 Homologação UX · 28/28 achados verificados (PDF em anexo)';
...
const info = await transporter.sendMail({
  ...
  attachments: [
    ...logoAttachments,
    {
      filename: 'Confronto_Homolog_UX_Relatorio4_AeroSuite.pdf',
      path: pdfPath,
