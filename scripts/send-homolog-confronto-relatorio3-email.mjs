#!/usr/bin/env node
/**
 * Envia Confronto Relatório 3 — somente se 100% conformidade (0 pendentes).
 * Uso: node scripts/send-homolog-confronto-relatorio3-email.mjs [--to email] [--dry-run]
 */
import fs from 'fs';
...
const { SCORE } = await import(`file://${path.join(root, 'docs/homolog_ux/confronto-relatorio3-data.mjs').replace(/\\/g, '/')}`);
...
const pdfPath = path.join(root, `docs/homolog_ux/Confronto_Homolog_UX_Relatorio3_${stamp}_verificado.pdf`);
const htmlPath = path.join(root, `docs/homolog_ux/emails/email-confronto-relatorio3-equipe-${stamp}.html`);

for (const f of [pdfPath, htmlPath]) {
  if (!fs.existsSync(f)) {
    console.error('Arquivo ausente:', f);
    console.error('Execute: node scripts/build-homolog-confronto-relatorio3-pdf.mjs');
    process.exit(1);
  }
}
...
const subject = 'Confronto Técnico — Relatório 3 Onboarding · 14/14 findings verificados (PDF em anexo)';
...
const info = await transporter.sendMail({
  ...
  attachments: [
    ...logoAttachments,
    {
      filename: 'Confronto_Homolog_UX_Relatorio3_AeroSuite.pdf',
      path: pdfPath,
