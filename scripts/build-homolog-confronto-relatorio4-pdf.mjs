#!/usr/bin/env node
/**
 * Gera Confronto Técnico — Homologação UX Relatório 4 (PDF + HTML).
 * Uso: node scripts/build-homolog-confronto-relatorio4-pdf.mjs
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { createRequire } from 'module';
import { META, SECTIONS, ITEMS, SCORE } from '../docs/homolog_ux/confronto-relatorio4-data.mjs';
...
const htmlOut = path.join(docDir, `Confronto_Homolog_UX_Relatorio4_${stamp}.html`);
const pdfOut = path.join(docDir, `Confronto_Homolog_UX_Relatorio4_${stamp}_verificado.pdf`);
...
function itemCard(it) {
  const rein = it.reincidente ? ' <span class="tag-rein">reincidência sanada</span>' : '';
  return `
<article class="item-card confronto-card">
  <div class="item-head">
    ...
    <span class="${statusClass(it.statusCode)}">${statusShort(it.statusCode)}</span>${rein}
  </div>
...
<section class="page">
  ...
  <div class="page-footer">
    <span>Aero Suite · Confronto UX Relatório 4</span>
...
.tag-rein { font-size: 7pt; color: #b45309; font-weight: 600; margin-left: 4px; }
...
<section class="cover">
  ...
  <div class="cover-inner">
    ...
    <div class="cover-badge">Confronto técnico · Relatório 4 · Confidencial</div>
...
<section class="page">
  ...
  <p class="lead">
  O Relatório 4 (17/jun/2026) registrou <strong>${META.reincidenciasRelatorio} reincidências</strong> entre ${META.retestadosRelatorio} achados retestados do Relatório 3,
  além de <strong>15 achados novos</strong> (C01–C04, P01–P11). Este documento confronta os <strong>28 achados</strong> com o estado após correção e verificação automatizada.
  </p>

  <div class="score-grid">
    <div class="score-card"><div class="score-val">${SCORE.total}</div><div class="score-label">Achados totais</div></div>
    <div class="score-card score-card--ok"><div class="score-val">${SCORE.verified}</div><div class="score-label">Verificados</div></div>
    <div class="score-card"><div class="score-val">${SCORE.pending}</div><div class="score-label">Pendentes</div></div>
    <div class="score-card score-card--ok"><div class="score-val">${SCORE.reincidentesOk}/${SCORE.reincidentesTotal}</div><div class="score-label">Reincidências sanadas</div></div>
  </div>
  ...
  <table class="metric-table">
    ...
    <tbody>
      <tr><td>Checks automatizados</td><td><strong>${SCORE.verificacaoAutomaticaPass}/${SCORE.verificacaoAutomaticaTotal}</strong></td></tr>
      <tr><td>Conformidade global</td><td><strong>${SCORE.pctConformidadeAchados}%</strong> (${SCORE.verified}/${SCORE.total})</td></tr>
      <tr><td>Críticos (F04, C01)</td><td><strong>${SCORE.pctCriticosResolvidos}%</strong></td></tr>
      <tr><td>Reincidências pós-Relatório 3</td><td><strong>${SCORE.pctReincidenciasSanadas}%</strong> (${SCORE.reincidentesOk}/${SCORE.reincidentesTotal})</td></tr>
    </tbody>
  </table>
