/**
 * Batch runner: writes .cdp-mcp-call-min.json per step, waits for .cdp-mcp-result.json
 * Usage: node batch-mcp-runner.mjs <viewId> <start> <end>
 */
import fs from 'fs';
...
const stepPath = path.join(dir, '.cdp-current-step.txt');
...
for (let n = start; n <= end; n++) {
  ...
  fs.writeFileSync(stepPath, String(n));
  ...
  try {
    ...
    if (rec.includes('"stopped":true')) process.exit(1);
  } catch (e) {
