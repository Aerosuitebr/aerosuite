/**
 * Envia logo King do Rio para a biblioteca de mídia do WordPress.
 * Uso: node run-upload-kingdorio-logo.mjs
 */
import fs from 'fs';
...
const logoPath = path.join(dir, 'static', 'portfolio', 'kingdorio-logo.png');
const outJson = path.join(dir, 'kingdorio-media.json');
...
const result = await page.evaluate(async ({ b64, fileName }) => {
  const existing = await wp.apiFetch({
    path: '/wp/v2/media?search=kingdorio-logo&per_page=5',
  });
  if (existing && existing.length) {
    const hit = existing.find((m) => m.slug && m.slug.indexOf('kingdorio') !== -1) || existing[0];
  ...
  form.append('title', 'King do Rio — Peças Aeronáuticas');
  form.append('alt_text', 'Logo King do Rio Peças Aeronáuticas');
...
}, { b64: base64, fileName: 'kingdorio-logo.png' });
