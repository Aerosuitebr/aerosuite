/**
 * Envia Relatório Executivo UX Sistema via WP Mail SMTP (mesmos destinatários do site).
 *
 * Uso (na raiz do repo):
 *   node scripts/build-ux-relatorio-sistema-pdf.mjs
 *   node docs/wordpress/run-send-ux-sistema-relatorio-email.mjs
 */
import fs from 'fs';
...
const pluginSlug = 'aerosuite-send-ux-sistema-relatorio-email';
...
const outPath = path.join(wpDir, 'ux-sistema-relatorio-email-result.json');

const pdfSrc = path.join(repoRoot, 'manuals/Relatorio_Executivo_UX_AeroSuite_Sistema.pdf');

function preparePluginBundle() {
  if (!fs.existsSync(pdfSrc)) {
    throw new Error(`PDF ausente. Rode: node scripts/build-ux-relatorio-sistema-pdf.mjs (${pdfSrc})`);
  }
  fs.copyFileSync(pdfSrc, path.join(pluginDir, 'Relatorio_Executivo_UX_AeroSuite_Sistema.pdf'));
}
...
async function findPluginRow(admin) {
  const plugins = await admin.evaluate(() =>
    [...document.querySelectorAll('tr[data-plugin*="aerosuite-send-ux-sistema-relatorio-email"]')].map((tr) => ({
...
async function activatePluginOnce(admin) {
  ...
  if ((await row.count()) === 0) throw new Error('Plugin UX sistema relatorio nao encontrado apos upload');
...
async function sendViaWordPress(admin) {
  ...
  let sent = await admin.evaluate(async () => {
    const r = await fetch('/wp-admin/admin-ajax.php?action=aerosuite_ux_sistema_relatorio_status', {
      credentials: 'include',
    });
  ...
  if (!Array.isArray(sent?.sent)) {
    const statusRes = await admin
      .goto(`${ORIGIN}/wp-admin/index.php?aerosuite_ux_sistema_relatorio_status=1`, { waitUntil: 'domcontentloaded' })
      .catch(() => null);
    if (statusRes?.ok()) {
...
async function main() {
  if (!fs.existsSync(storagePath)) {
    console.error('UX_SISTEMA_RELATORIO_EMAIL_FAIL missing wp-storage.json');
    process.exit(1);
  }
  ...
  const result = {
    ...
    subject: '[Aero Suite] Relatório Executivo UX Sistema — 100% aderente (12/12 itens resolvidos)',
  ...
  console.log(ok ? 'UX_SISTEMA_RELATORIO_EMAIL_OK' : 'UX_SISTEMA_RELATORIO_EMAIL_WARN', JSON.stringify(result.sent, null, 2));
  process.exit(ok ? 0 : 2);
}

main().catch((e) => {
  console.error('UX_SISTEMA_RELATORIO_EMAIL_FAIL', e.message);
  process.exit(1);
});
