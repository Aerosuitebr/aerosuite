/**
 * Publica analytics de conversão + Soluções/Sobre (requer wp-storage.json).
 */
import fs from 'fs';
...
const deployCode = fs.readFileSync(path.join(dir, '.deploy-conversion-once.js'), 'utf8');
...
fs.writeFileSync(path.join(dir, 'conversion-deploy-result.json'), JSON.stringify(result, null, 2));
