/** Execute steps via MCP file handshake + record. Agent runs MCP; this script drives the loop.
 * Usage: node .cdp-mcp-file-loop.mjs 2 29 2effaf
 * For each step writes .cdp-mcp-pending.json and waits for .cdp-mcp-result-now.json
 */
import fs from 'fs';
...
for (let n = start; n <= end; n++) {
  ...
  fs.writeFileSync(pendingPath, JSON.stringify({ step: n, method: c.method, params: c.params, viewId }));
