/**
 * Run invoke steps 0-29 via Playwright CDP (same Runtime.evaluate as browser_cdp).
 * Usage: node .cdp-run-all-via-playwright.mjs [viewIdHint]
 */
import fs from 'fs';
...
const invokeViewId = process.argv[2] || 'd15c6f';
const requestedViewId = 'a9930e';
const activeViewId = process.argv[3] || 'b45110';
...
const summary = {};
const errors = [];
let restartFrom0 = false;

for (let i = 0; i <= 29; i++) {
  if (restartFrom0 && i > 3) continue;
  ...
  try {
    ...
    fs.writeFileSync(path.join(dir, `.cdp-step-${i}-pw-value.json`), JSON.stringify(value));
