/**
 * Loop steps: writes .cdp-current-step.txt and waits for .cdp-mcp-done-now.json
 * Usage: node .cdp-agent-batch-loop.mjs 84ede5 1 29
 */
import fs from 'fs';
...
const stepFile = path.join(dir, '.cdp-current-step.txt');
...
for (let n = start; n <= end; ) {
  execSync(`node .cdp-agent-mcp-call.mjs prepare ${n} ${viewId}`, { cwd: dir, stdio: 'pipe' });
  if (fs.existsSync(doneFile)) fs.unlinkSync(doneFile);
  fs.writeFileSync(stepFile, String(n));
